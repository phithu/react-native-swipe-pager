...
- (void)layoutSubviews {
    ...
    if (_reactPageViewController) {
        [self setTransitionStyle:_transitionStyle];
...
- (void) embed:(int) index {
    _initialPage = index;
    _transitionStyle = index % 2 != 0 ? UIPageViewControllerTransitionStylePageCurl : UIPageViewControllerTransitionStyleScroll;
    [self embed];
}
